...
function installAuthMiddleware(app) {
  // Our own mini auth server middleware
  app.get('*', async (req, res, next) => {
    const cookie = new Cookies(req?.headers?.cookie, { path: '/' });
    const username = String(cookie.get(process.env.PUBLIC_ENV__USERNAME_READ_COOKIE) ?? "");
    const password = String(cookie.get(process.env.PUBLIC_ENV__AUTH_READ_COOKIE) ?? "");
    const remember = !!cookie.get(process.env.PUBLIC_ENV__AUTH_REMEMBER_COOKIE);
    req.user = remember
